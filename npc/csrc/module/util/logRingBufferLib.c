...
logRingBuffer *createLogRingBuffer(const int bufferSize) {
    logRingBuffer *newLogRingBuffer = (logRingBuffer *)malloc(sizeof(logRingBuffer));
    if (!newLogRingBuffer) {
        echoError(MALLOC_ERROR);
    }

    newLogRingBuffer->logArr = (char **)calloc(bufferSize,sizeof(char *));
...
char **obtainLogRingBuffer(logRingBuffer *inputLogRingBuffer, const uint64_t readLength){
    char **newLogArr= (char **)malloc(readLength* sizeof(char *));
    if(!newLogArr){
        echoError(MALLOC_ERROR);
    }
