...
__attribute__((noreturn)) static void echoError(const char *errorMessage) {
    if (errno) {
        perror(errorMessage);
        exit(1);
    }
...
logStack *createLogStack(int initialEleCount) {
    ...
    logStack *newLogStack = (logStack *)malloc(sizeof(logStack));
    if (!newLogStack) echoError(MALLOC_ERROR);

    newLogStack->logArr = (char **)calloc(initialEleCount, sizeof(char *));
...
// 压栈
void shiftLogStack(logStack *inputLogStack, const char *fmt, ...) {
    if (inputLogStack->eleCount+1>inputLogStack->capacity) {
        char **newLogArr = (char **)realloc(inputLogStack->logArr,2 * inputLogStack->capacity * sizeof(char *));
        if (!newLogArr) {
          free(inputLogStack->logArr);
          free(inputLogStack);
          echoError(MALLOC_ERROR);
    ...
    char *currentLog = (char *)malloc(requiredLen + 1); // +1 for null terminator
    if (!currentLog) {
      echoError(MALLOC_ERROR);
    }
...
void unshiftLogStack(logStack *inputLogStack, int adjustMemory) {
    ...
    if (adjustMemory) {
        char **newLogArr = (char **)realloc(inputLogStack->logArr, inputLogStack->eleCount * sizeof(char *));
        if (!newLogArr) {
          free(inputLogStack->logArr);
          free(inputLogStack);
          echoError(MALLOC_ERROR);
